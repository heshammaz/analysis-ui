...
export default class Histogram extends Component {
  ...
  renderBars (chartData, barWidth, maxBarHeight, color) {
    const rgba = toRgbaString(chroma(color).alpha(0.4))
...
function computeChartData (data, maxAccessibility, isochroneCutoff) {
  ...
  for (let iteration = 0; iteration < data.length; iteration++) {
    const marginals = data[iteration]
    let val = 0
    // marginal[10] is the marginal access from 10 to 11 minutes, so it shouldn't be included
    // when isochroneCutoff === 10, hence use <
    for (let minute = 0; minute < isochroneCutoff; minute++) {
      val += marginals[minute]
    }
...
function toRgbaString (chromaObject) {
  const [r, g, b, a] = chromaObject.rgba()
  return `rgba(${r}, ${g}, ${b}, ${a})`
}
